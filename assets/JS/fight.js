...
function getPotion() {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        document.getElementById("footer__button-heal").innerHTML =
          xhr.responseText;
      } else {
        console.error("Error:", xhr.status);
      }
...
function playerHeal() {
  ...
  if (currentTime - lastCheckTime >= 2200) {
    const xhr = new XMLHttpRequest();
    ...
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        ...
        if (damage == 77777) {
          ...
          var playerAttackAudio = new Audio("assets/Sound/sword.mp3");
          playerAttackAudio.volume = 0.6;
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        ...
        const sound = new Audio("assets/Sound/" + response.sound);
        if (damage == 66666) {
          window.location.href = "die.php";
