...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($itemresult) {
        ...
        if ($itemFound) {
            ...
            $price = round($itemPrice['price'] * ($lvl));
            ...
            if ($row['balance']>=$price) {
             ...
             if ($result) {
                $removequery = "SELECT shop FROM users WHERE id = $userId";
                $removeresult = mysqli_query($link, $removequery);

                if ($removeresult) {
                    $removerow = mysqli_fetch_assoc($removeresult);
                    $removeitems = json_decode($removerow['shop'], true);
                    ...
                    $updateQuery = "UPDATE users SET shop = '$updatedInventory' WHERE id = $userId";
